...
async function writeBytecode(bytecode: string) {
    ...
    await filesystem.writeFile(filePath, bytecode, {
        encoding: 'utf8',
        flag: 'w',
    })
}
...
async function writeFactoryDeployerTransaction(
    ...
    if (process.env.PK) {
        signer = new ethers.Wallet(process.env.PK)
    } else {
...
export async function estimateDeploymentTransaction(
    ...
    const gasLimit = await provider.estimateGas({ data, from: signer.address })
    ...
    console.log({
        requiredFunds: ethers.utils.formatUnits(
...
export async function createDeploymentTransaction(
    ...
    await ensureDirectoryExists(
        path.join(__dirname, '..', 'artifacts', `${chainId}`),
    )
